...
export class RefinementService {
    ...
    constructor(
        ...
        this.logger.log(`Perplexity AI ${this.configService.usePerplexityAi ? 'habilitado' : 'desabilitado'}`);
    }
    ...
    async refineLayout(layoutData: LayoutData): Promise<RefinedLayout[]> {
        try {
            ...
            if (this.configService.usePerplexityAi && apiKey) {
    ...
    private async refineLayoutWithPerplexity(
        ...
        try {
            ...
            const perplexityResponse = await firstValueFrom(
            ...
            if (!perplexityResponse.data || !perplexityResponse.data.choices || !perplexityResponse.data.choices[0]) {
                this.logger.error('Resposta da API do Perplexity não contém os campos esperados');
                this.logger.debug(`Resposta: ${JSON.stringify(perplexityResponse.data)}`);
